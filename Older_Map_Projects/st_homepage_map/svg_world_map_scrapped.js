...
let pops_by_lang = {
    ...
    "Arabic": {
        ...
        "Chad": 3332000,
        "Colombia": 38000,
        "Democratic Republic of Congo": 27000,
        "Côte d'Ivoire": 79000,
        "Cuba": 21000,
        "Denmark": 53000,
        "Djibouti": 87000,
    ...
    "French": {
        ...
        "Chad": 17000,
        "Democratic Republic of Congo": 87000,
        "Republic of Congo": 12000,
        "Côte d'Ivoire": 41000,
        "Egypt": 93000,
        "France": 54028000,
        "French Guiana": 24000,
    ...
    "Hindi": {
        "Austria": 23000,
        "Canada": 482000,
        "Democratic Republic of Congo": 190000,
        "Cuba": 34000,
        "Finland": 11000,
        "France": 275000,
        "Germany": 195000,
    ...
    "Swahili": {
        ...
        "Burundi": 13000,
        "Canada": 342000,
        "Democratic Republic of Congo": 54000,
        "Germany": 248000,
        "Italy": 126000,
        "Kenya": 286000,
        "Mozambique": 23000,
...
class Language {
    ...
    constructor(language_name) {
        ...
        this.population_by_country = [];        

        //this.get_language_data_for_each_country();        
    }

    get_language_data_for_each_country() {  
                
        for (let i=0; i<svg_countries.length; i++) {  
            //children[0] is the path element within the svg           
            let svg_country_name = svg_countries[i].children[0].getAttribute('title');            

            if (pops_by_lang[this.name][svg_country_name]) {                
                let language_pop = pops_by_lang[this.name][svg_country_name];

                let total_country_pop;
                let gdp_for_lang;                
                for (let j=0; j<data_by_country.length; j++) {
                    if (data_by_country[j].country == svg_country_name) {
                        total_country_pop = data_by_country[j].population;

                        gdp_for_lang = (isNaN(data_by_country[j].gdpPerCapita)) ? 0 : Math.round(data_by_country[j].gdpPerCapita);                        
                    }

                }
                let language_percent = (language_pop/total_country_pop)*100;
                let language_pop_by_country = {
                    Name: this.name,
                    Country: svg_country_name,
                    Population: language_pop,
                    Percent: language_percent,
                    PerCapitaGDP: gdp_for_lang
                };

                this.population_by_country.push(language_pop_by_country);                                                                
            }
        } 
        this.sort_countries_by_language_pop();       
    }

    sort_countries_by_language_pop() {
        this.population_by_country.sort(function(a,b) {
            return b.Population - a.Population;
        })
    }

    assign_colors(primary_color, secondary_color) {
...
class Country {
    ...
    constructor(name, svg_element) {        
        ...
        this.other = 100; // in %   
        this.set_all_languages();
    }
    
    set_selected_languages() {
        let temp_array = [];         

        for (let i=0; i<subject.selected_languages.length; i++) {
            let population_by_country = subject.selected_languages[i].population_by_country;
            for (let j=0; j<population_by_country.length; j++) {
                                
                if (population_by_country[j].Country == this.name) {                    
    
                    temp_array.push(population_by_country[j]);                       
                    this.other -= population_by_country[j].Percent;
                }
            }            
        }
        this.selected_languages = temp_array;
        this.sort(this.selected_languages);        
    }

    set_all_languages() {
        for(let i=0; i<subject.languages.length; i++) {                
            //console.log('languages' + subject.languages);       
            let population_by_country = subject.languages[i].population_by_country;
            for(let j=0; j<population_by_country.length; j++) {
                let lang_data_by_country = population_by_country[j];
                let country_name = lang_data_by_country.Country;
                if(country_name == this.name) {
                    this.all_languages.push(lang_data_by_country);
                }
            }
        }
        this.sort(this.all_languages);
    }
...
class Subject {
    ...
    constructor(languages, countries) {  
        //lists of all languages and all countries represented as js objects              
        this.countries = countries; 
        this.languages = [];
        //languages that have been selected in the selectbox
        this.selected_languages = [];        
        this.selected_languages_in_panel = [];
        this.selected_langs_counter = this.selected_languages.length;
    }
    ...
    get_language(language_name) {
        for(let i=0; i<this.selected_languages.length; i++) {
            let language = this.selected_languages[i];
    ...
    add_language(language) {
        ...
        language.get_language_data_for_each_country();         
    }
    ...
    select_language(language) {
        ...
        for(let i=0; i<this.countries.length; i++) {
            ...
            country.set_selected_languages();
        }
        this.update_observers();                      
    }

    //*** remove language from selected languages
    remove_language(language_name) {
        let temp_array = [];        
        for (let i=0; i<this.selected_languages.length; i++) {
            let lang = this.selected_languages[i];
            
            if (lang.name == language_name) continue;
    ...
    update_language_panel() {          
        for (let i=0; i<this.selected_languages.length; i++)  {
            ...
            let selected_lang = this.selected_languages[i];              
            if(this.selected_languages_in_panel.includes(selected_lang)) continue;
            else {                
                add_language_data_to_panel(selected_lang, selected_lang.primary_color, selected_lang.secondary_color);
                this.selected_languages_in_panel.push(selected_lang); 
            }                
        }
    }
    ...
    update_selectbox() {
        ...
        for (language in pops_by_lang) {
            ...
            for(let i=0; i<this.selected_languages.length; i++) {
                ...
                let selected_lang = this.selected_languages[i].name;    
                if (language == selected_lang) lang_is_selected = true;
            }
        
            if (lang_is_selected) continue;
            else choices.setChoices([{ value: language, label: language }]);            
        }  
    } 
}
...
function set_choices_selectbox() {
    ...
    for (language in pops_by_lang) {                
        ...
        let my_value = language;
        let my_label = language;
    ...
    let select_language_btn = document.getElementById('choices-input-btn');
    select_language_btn.addEventListener(
    'click',
    function(e) {
        ...
        for (let i = 0; i < selected_langs.length; i++) {
            ...
            let primary_color = color_palette_array[i][0];
            let secondary_color = color_palette_array[i][1];

            let selected_lang = new Language(language_name);
            selected_lang.assign_colors(primary_color, secondary_color);
            subject.select_language(selected_lang);                                              
        }    
...
function generate_country_details_panel() {
    ...
    for(let i=0; i<subject.countries.length; i++) {
        ...
        let svg_country = subject.countries[i].svg_element; 

        svg_country.addEventListener(
            'click',
            function(e) {
                let parent_elem = document.getElementById('svg-container');
                ...
                */
                ...
                country_name.innerHTML = svg_country.children[0].getAttribute('title'); 
                ...
                for(let i=0; i<country.all_languages.length; i++) {                                
                    let language = country.all_languages[i];                    
                    let language_population = language.Population;
                    ...
                    language_data_for_country.innerHTML = `${language.Name}: ${language_population}`;
...
function set_up_tooltips() {
    ...
    let panZoomMap = svgPanZoom(svgElement, {
...
function display_lang_data_on_map() {    
    ...
    for (let i = 0; i < subject.countries.length; i++) {
        ...
        let country = subject.countries[i];
        let population_by_language = country.selected_languages;  
        console.log(population_by_language);      

        let offset = 0;
        for(let j=0; j<population_by_language.length; j++) {

            let language_data = population_by_language[j];               
            let language_percent = (isNaN(language_data.Percent)) ? 0 : Math.ceil(language_data.Percent);            
            let color = subject.get_language(language_data.Name).primary_color;                      
        ...
        //add 'other' stops    
        let other = (isNaN(country.other)) ? 0 : country.other;
        ...
        new_stop_4.setAttribute("offset", `${offset + other}%`);
...
function add_language_data_to_panel(selected_lang) {        
    ...
    //add language details box (seen when row is hovered over)
    ...
    //add data for each country (by language) to detailed view
    let countries_by_lang = selected_lang.population_by_country;  
    let gdp_by_lang = 0; 
    for (let i=0; i<countries_by_lang.length; i++) {
        
        let language_details = document.createElement('div');
        language_details.setAttribute('class', 'language-details');                


        let country_name = countries_by_lang[i].Country;        
        let language_pop = countries_by_lang[i].Population;        
        let country_gdp_for_lang = (isNaN(countries_by_lang[i].PerCapitaGDP)) ? 0 : countries_by_lang[i].PerCapitaGDP;

        gdp_by_lang += (Math.round(country_gdp_for_lang) * language_pop); 
                
        language_details.innerHTML = `${country_name} : ${language_pop}, ${country_gdp_for_lang}`;               
    ...
    new_online_presence_entry.innerHTML = pops_by_lang[selected_lang.name]["Total Speakers"];    

    //add gdp of market data
    let new_gdp_entry = document.createElement('button');
    new_gdp_entry.setAttribute('class', 'panel-data-row gdp-btn');    
    

    let gdp_num_length = gdp_by_lang.toString().length;
    let gdp_in_billions = Math.round(gdp_by_lang/1000000000);
    let gdp_in_trillions = Math.round(gdp_by_lang/1000000000000); 

    gdp_by_lang = (gdp_num_length >= 13) ? `${gdp_in_trillions} T` : `${gdp_in_billions} B`;    
    new_gdp_entry.innerHTML = gdp_by_lang;
    ...
    remove_entry_btn.addEventListener(
        ...
        function(e) {        
            ...
            subject.remove_language(selected_lang.name);
            subject.update_observers();
    ...
    new_entry_container.addEventListener(
     ...
     new_entry_container.addEventListener(
      'mouseout',
      function(e) {
          new_entry_container.style.backgroundColor = '#efefef';
...
for(language in pops_by_lang) {          
    if(language != "Grand Total") {
        let new_lang = new Language(language, primary_color, secondary_color);
...
/////////////////////////////////////
//Eevent definition for svg countries
/////////////////////////////////////
